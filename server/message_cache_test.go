...
func testCacheMessages(t *testing.T, c *messageCache) {
	...
	// mytopic: since all
	messages, _ := c.Messages("mytopic", sinceAllMessages, false, 0, nil)
	...
	// mytopic: since none
	messages, _ = c.Messages("mytopic", sinceNoMessages, false, 0, nil)
	require.Empty(t, messages)

	// mytopic: since m1 (by ID)
	messages, _ = c.Messages("mytopic", newSinceID(m1.ID), false, 0, nil)
	...
	// mytopic: since 2
	messages, _ = c.Messages("mytopic", newSinceTime(2), false, 0, nil)
	require.Equal(t, 1, len(messages))
	require.Equal(t, "my other message", messages[0].Message)

	// mytopic: latest
	messages, _ = c.Messages("mytopic", sinceLatestMessage, false, 0, nil)
	require.Equal(t, 1, len(messages))
	require.Equal(t, "my other message", messages[0].Message)
	...
	// example: since all
	messages, _ = c.Messages("example", sinceAllMessages, false, 0, nil)
	...
	messages, _ = c.Messages("doesnotexist", sinceAllMessages, false, 0, nil)
	require.Empty(t, messages)
}
...
func testCacheMessagesScheduled(t *testing.T, c *messageCache) {
	...
	messages, _ := c.Messages("mytopic", sinceAllMessages, false, 0, nil) // exclude scheduled
	require.Equal(t, 1, len(messages))
	require.Equal(t, "message 1", messages[0].Message)

	messages, _ = c.Messages("mytopic", sinceAllMessages, true, 0, nil) // include scheduled
...
func testCacheMessagesSinceID(t *testing.T, c *messageCache) {
	...
	messages, _ := c.Messages("mytopic", newSinceID(m2.ID), false, 0, nil)
	require.Equal(t, 3, len(messages))
	...
	messages, _ = c.Messages("mytopic", newSinceID(m2.ID), true, 0, nil)
	...
	messages, _ = c.Messages("mytopic", newSinceID("doesntexist"), true, 0, nil)
	...
	messages, _ = c.Messages("mytopic", newSinceID(m7.ID), false, 0, nil)
	require.Equal(t, 0, len(messages))

	// Case 5: Since ID exists and is last message (-> Return no messages), include scheduled
	messages, _ = c.Messages("mytopic", newSinceID(m7.ID), true, 0, nil)
	require.Equal(t, 2, len(messages))
...
func testCachePrune(t *testing.T, c *messageCache) {
	...
	messages, err := c.Messages("mytopic", sinceAllMessages, false, 0, nil)
	require.Nil(t, err)
	require.Equal(t, 1, len(messages))
	require.Equal(t, "my other message", messages[0].Message)
}
...
func TestSqliteCache_Migration_From1(t *testing.T) {
	...
	// 11!
	messages, err = c.Messages("mytopic", sinceAllMessages, true, 0, nil)
	require.Nil(t, err)
...
func testDeleteScheduledBySequenceID(t *testing.T, c *messageCache) {
	...
	messages, err = c.Messages("othertopic", sinceAllMessages, true, 0, nil)
	require.Nil(t, err)
	require.Equal(t, 1, len(messages))
